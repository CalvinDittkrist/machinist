...
- Work on a short-lived branch and open a PR against `main`. Never push directly to `main`.
...
- Use squash merges for ordinary changes. Preserve ancestry with a merge commit when importing a complete upstream range. Agents prepare these PRs but never merge them.
